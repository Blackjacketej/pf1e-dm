...
try:
    ...
    print("Warning: could not parse resolutions file (truncated)", file=sys.stderr)
...
for i, e in enumerate(notes[:12], 1):
    ...
    rank_str = f"#{rp}" if rp else "unranked"
    print(f"{i}. #{e['id']} | {e['kind'].upper()} | severity={e['severity'] or 'N/A'} | rank={rank_str}")
    print(f"   Created: {e['created_at']}")
    if e['scene_char']:
        print(f"   {e['scene_char']}")
    print(f"\n   Body:")
    print(e['body'])
    print()
