...
export default function LevelUpWizard({
  ...
  // Filter feats based on prerequisites (simplified check)
  const availableFeats = useMemo(() => {
    return featsData.filter(feat => {
      const searchMatch = feat.name.toLowerCase().includes(searchTerm.toLowerCase());
      // Very simplified - in production would check prerequisites properly
      return searchMatch;
    });
  }, [searchTerm]);
  ...
  const availableSpells = useMemo(() => {
    ...
    return spellsData
      .filter(spell => {
        ...
        const hasClass = spell.level?.[classKey] !== undefined;
        return matchesSearch && hasClass;
      })
      .slice(0, 50);
  }, [char.class, searchTerm]);
  ...
  const toggleFeatSelection = (featName) => {
    setSelectedFeats(prev => {
      ...
      if (prev.length < totalFeatsToSelect) {
      ...
      <div style={{ maxHeight: '350px', overflowY: 'auto' }}>
        {availableFeats.map(feat => (
          <button
            ...
            disabled={!selectedFeats.includes(feat.name) && selectedFeats.length >= totalFeatsToSelect}
            style={{
              display: 'block',
              width: '100%',
              textAlign: 'left',
              background: selectedFeats.includes(feat.name) ? THEME.gold : THEME.panel,
              color: selectedFeats.includes(feat.name) ? THEME.bg : THEME.text,
              border: `1px solid ${THEME.border}`,
              padding: '10px',
              marginBottom: '8px',
              borderRadius: '4px',
              cursor: 'pointer',
              opacity: !selectedFeats.includes(feat.name) && selectedFeats.length >= totalFeatsToSelect ? 0.5 : 1,
            }}
          >
